...
const ESP32_BASE_URL =
  process.env.NEXT_PUBLIC_ESP32_BASE_URL || "http://10.97.54.34";
  ...
  const setRelayState = async (
    ...
    try {
      const res = await fetch(
        `${ESP32_BASE_URL}/relay?ch=${channel}&state=${state}`
      );
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {displayDevices.map((item) => {
            const loadFactor =
              ts.field4 > 0 ? Math.round((item.power / ts.field4) * 100) : 0;
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between mt-4">
                  <button
                    onClick={() => setRelayState(channel, "on")}
                    className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded"
                  >
                    ON
                  </button>
                  <button
                    onClick={() => setRelayState(channel, "off")}
                    className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded"
                  >
                    OFF
                  </button>
                  <button
                    onClick={() => setRelayState(channel, "auto")}
