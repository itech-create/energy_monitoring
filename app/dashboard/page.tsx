...
// --- ESP32 Local API Config --- // NEW
const ESP32_BASE_URL = process.env.NEXT_PUBLIC_ESP32_BASE_URL || "http://10.97.54.34"; // Change to ESP32 IP
            ...
            return (
              <div key={item.id} className="bg-gray-900 p-6 rounded-xl shadow border border-gray-800 space-y-4 relative">
                ...
                <div className="flex justify-between mt-4">
                  <button
                    onClick={() => setRelayState(item.field === 1 ? 1 : item.field === 5 ? 2 : 3, "on")}
                    className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded"
                  >
                    ON
                  </button>
                  <button
                    onClick={() => setRelayState(item.field === 1 ? 1 : item.field === 5 ? 2 : 3, "off")}
                    className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded"
                  >
                    OFF
                  </button>
                  <button
                    onClick={() => setRelayState(item.field === 1 ? 1 : item.field === 5 ? 2 : 3, "auto")}
